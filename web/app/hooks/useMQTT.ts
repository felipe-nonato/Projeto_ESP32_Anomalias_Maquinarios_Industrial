    ...
    useEffect(() => {
        ...
        client.on('message', async (receivedTopic: string, message: Buffer) => {
            try {
                ...
                const savedDevices: string | null =
                    localStorage.getItem('devices')
                let deviceName: string = `Dispositivo ${mqttPayload.device_id.substring(
                    0,
                    8
                )}`
                ...
                if (savedDevices) {
                    try {
                        ...
                        const device: Device | undefined = devices.find(
                            (d: Device) => d.id === mqttPayload.device_id
                        )
                        if (device) {
                ...
                try {
                    await fetch(`${API_URL}/api/readings`, {
                        ...
                        body: JSON.stringify({
                            device_id: mqttPayload.device_id,
                            label: mqttPayload.label,
                            score: mqttPayload.score,
                        }),
                    })
                    console.log('Dados salvos no banco de dados')
                } catch (apiError) {
                    console.error('Erro ao salvar no banco de dados:', apiError)
                }
